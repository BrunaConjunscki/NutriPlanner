...
const NovaAntropometriaModal = ({ isOpen, onRequestClose, pacienteId, consultaId }) => {
    ...
    const handleSubmit = async () => {
        if (validateForm()) {
            try {
                const response = await axios.post(`http://localhost:8000/api/pacientes/${pacienteId}/consultas/${consultaId}/antropometrias`, {
                    antropometria
        ...
        <>
            <Modal isOpen={isOpen} onRequestClose={handleClose} className="modal-content-novo"
                ...
                <div className="modal-body">
                    ...
                    {showSuccess && (
                        <Modal isOpen={showSuccess} className="success-modal"
                               overlayClassName="modal-overlay-novoPaciente">
                            <div className="success-content">
                            <h3>Antropometria Cadastrada com Sucesso!</h3>
                            </div>
                        </Modal>
                    )}
...
export default NovaAntropometriaModal;
